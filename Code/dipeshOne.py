...
def createMergeFile(startDate, EndDate, weatherData):
    engine = create_engine("sqlite:///../Data/megawatt.sqlite")

    newdf = DataFrame(engine.execute("SELECT * FROM demand where date >'"+startDate+"'and date < '"+EndDate+"' and name='ISNE'").fetchall())
...
createMergeFile('2017-01-01','2018-12-01','2016-2018_MA_weather_data.csv')
